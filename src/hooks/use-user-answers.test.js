...
describe(`useUserAnswers test`, () => {
  ...
  it(`useUserAnswers should return array with 3 elements`, () => {
    const {result} = renderHook(() => useUserAnswers(questionMock, jest.fn()));
  ...
  it(`useUserAnswers should correctly change state`, () => {
    const {result} = renderHook(() => useUserAnswers(questionMock, jest.fn()));
  ...
  it(`useUserAnswers should call onAnswer`, () => {
    const onAnswer = jest.fn();
    const {result} = renderHook(() => useUserAnswers(questionMock, onAnswer));
