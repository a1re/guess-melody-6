...
describe(`Reducers work correctly`, () => {
  it(`Reducer without additional parameters should return initial state`, () => {
    expect(gameProcess(undefined, {})).toEqual({step: 0, mistakes: 0});
  });

  it(`Reducer should increment current step by a given value`, () => {
    const state = {step: 0, mistakes: 0};

    expect(gameProcess(state, incrementStep())).toEqual({step: 1, mistakes: 0});
    ...
    expect(gameProcess(state, nonIncrementStepAction)).toEqual({step: 0, mistakes: 0});
  });
  ...
  it(`Reducer should increment number of mistakes by a given value`, () => {
    ...
    expect(gameProcess(state, incrementMistakeAction)).toEqual({step: 0, mistakes: 1});
    ...
    expect(gameProcess(state, nonIncrementMistakeAction)).toEqual({step: 0, mistakes: 0});
  });
